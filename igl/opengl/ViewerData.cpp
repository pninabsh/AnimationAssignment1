...
{
	if (parent == nullptr) {
		return Tout.matrix() * roty.matrix() * rotx.matrix() * roty2.matrix() * Tin.matrix() * scale.matrix();
	}
	return parent->MakeTrans() * Tout.matrix() * roty.matrix() * rotx.matrix() * roty2.matrix() * Tin.matrix() * scale.matrix();
}
