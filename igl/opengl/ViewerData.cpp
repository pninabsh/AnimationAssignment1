...
Eigen::Matrix3f igl::opengl::ViewerData::getRotationOfParents()
{
	if (parent == nullptr) {
		return rot.rotation();
	}
	return parent->getRotationOfParents() * rot.rotation();
}

Eigen::Matrix4f igl::opengl::ViewerData::getTranslationOfParents()
{
	if (parent == nullptr) {
		return Tout.matrix() * Tin.matrix();
	}
	return parent->getTranslationOfParents() * Tout.matrix() * Tin.matrix();
}
