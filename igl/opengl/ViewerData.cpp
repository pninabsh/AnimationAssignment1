...
{
	if (parent == nullptr) {
		return Tout.matrix() * roty2.matrix() * roty.matrix() * rotx.matrix() * Tin.matrix() * scale.matrix();
	}
	return parent->MakeTrans() * Tout.matrix() * roty2.matrix() * roty.matrix() * rotx.matrix() * Tin.matrix() * scale.matrix();
}
